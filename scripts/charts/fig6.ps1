...
.SYNOPSIS
figD.ps1
Generate Figure_6.xlsx -- forward cumulative share MultiLine (4 measures).

Inputs:
  data/outputs/charts/fig6_multiline.csv   (from 09_prepare_chart_data.py)
  cgd-general/chart-templates/Chart-template-MultiLine.xlsx

Output:
  data/outputs/charts/Figure_6.xlsx
...
$ML_COLS = @{
    "fig6" = @{
        ...
        "ghg_excLUC" = @{ A = @("annex2_OWID","annex2_PRIMAP","","");                                B = @("annex1_OWID","annex1_PRIMAP","","");                                C = @("non_annex1_OWID","non_annex1_PRIMAP","","") }
        "ghg_incLUC" = @{ A = @("annex2_OWID","annex2_PRIMAP","","");                                B = @("annex1_OWID","annex1_PRIMAP","","");                                C = @("non_annex1_OWID","non_annex1_PRIMAP","","") }
    }
}

$ML_TITLES = @{
    "fig6" = @{
        "co2_excLUC" = "Cumulative CO2 emissions excluding LULUCF since 1850, different end points"
        "co2_incLUC" = "Cumulative CO2 emissions including from LULUCF since 1850, different end points"
        "ghg_excLUC" = "Cumulative GHG emissions excluding LULUCF since 1850, different end points"
        "ghg_incLUC" = "Cumulative GHG emissions including LULUCF since 1850, different end points"
    }
}
...
Write-Host "`nFig 6: MultiLine (4 measures) ..."
...
Save-Close $wbML "$outDir\Figure_6.xlsx"
