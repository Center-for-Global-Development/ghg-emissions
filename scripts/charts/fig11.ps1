<#
.SYNOPSIS
figI.ps1 — Cumulative per-capita emissions dumbbell, 1990–2024 baseline (Figure 11 in paper).

Reads fig11_percapita_cumulative_1990.csv (built by 09_prepare_chart_data.py).
Three dots per row: Method A (TS), Method B (EN), Method C (PYW) + abs_share triangle.

Inputs:
  data/outputs/charts/fig11_percapita_cumulative_1990.csv
  cgd-general/chart-templates/Chart-template-Dumbbell.xlsx

Output:
  data/outputs/charts/Figure_11.xlsx
...
function Build-FigH([string]$csvFile, [string]$outFile, [string]$title) {
    ...
    for ($r2 = $startRow; $r2 -le $endRow; $r2++) {
        $ws.Cells($r2,  7).Formula = "=IF(COUNT(C${r2}:E${r2})<=1,NA(),AGGREGATE(5,6,C${r2}:E${r2}))"
        $ws.Cells($r2,  8).Formula = "=IF(COUNT(C${r2}:E${r2})<=1,0,AGGREGATE(4,6,C${r2}:E${r2})-AGGREGATE(5,6,C${r2}:E${r2}))"
        $ws.Cells($r2,  9).Value2  = 0
        $ws.Cells($r2, 10).Formula = "=IF(ISNA(G${r2}),NA(),G${r2}+H${r2})"
        $ws.Cells($r2, 11).Formula = "=IF(ISNA(G${r2}),NA(),G${r2})"
        $ws.Cells($r2, 12).Formula = "=IF(ISNA(G${r2}),NA(),J${r2})"
        $ws.Cells($r2, 13).Formula = "=IF(ISNA(G${r2}),NA(),A${r2})"
        $ws.Cells($r2, 14).Formula = "=M${r2}"
    }
    ...
    Format-DataRange $ws $startRow $endRow 3 6 "0.0"
    Format-Sheet     $ws $startRow $endRow 14
...
try {
    ...
    Write-Host "`nFigure 11: 1990-2024 baseline ..."
    Build-FigH "fig11_percapita_cumulative_1990.csv" "Figure_11.xlsx" `
        "Cumulative per-capita emissions as % of world total (1990${em}2024)"
